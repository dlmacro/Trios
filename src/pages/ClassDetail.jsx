...
import {
  ...
  Edit, Eye, X, Clock, Settings, BookOpen,
  Save, Plus, Trash2, ToggleLeft, ToggleRight, Globe, QrCode,
...
function StatusBadge({ status }) {
  ...
  return <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${cls[status] || 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}`}>{status}</span>;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Data starts at row index 3 (row 0=meta, row 1=blank, row 2=headers)
...
export default function ClassDetail() {
  ...
  const [subjectTeachers, setSubjectTeachers] = useState([]); // junction records
  const [allTeachers, setAllTeachers] = useState([]);
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState('students');

  // Access control — derived reactively from user + teacher state
  const isAdmin = user?.role === 'admin' || user?.role === 'principal';
  const canManage = isAdmin || (teacher && teacher.name === user?.name);
  ...
  const [classForm, setClassForm] = useState({ classTeacherId: '', capacity: 40, academicYear: '2025', isEnglishMedium: false });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [classRecord, allStudents, allTeachersData, allSubjects, classSubjectTeachers] = await Promise.all([
        db.classes.get(classId),
        db.students.toArray(),
        db.teachers.toArray(),
        db.subjects.toArray(),
        db.subjectTeachers.where('classId').equals(classId).toArray(),
      ]);

      if (!classRecord) { navigate('/classes'); return; }
      setCls(classRecord);
      setAllTeachers(allTeachersData);

      const classTeacher = allTeachersData.find(t => t.id === classRecord.classTeacherId);
      setTeacher(classTeacher || null);

      const enrolled = allStudents.filter(s =>
        Number(s.grade) === classRecord.grade && s.parallel === classRecord.parallel
      );
      ...
      const tt = await db.timetable.where('classId').equals(classId).toArray();
      setTimetable(tt);

      const subs = allSubjects.filter(s => s.grade === classRecord.grade);
      setSubjects(subs);
      setSubjectTeachers(classSubjectTeachers);

      setClassForm({
        classTeacherId: classRecord.classTeacherId ? String(classRecord.classTeacherId) : '',
        capacity: classRecord.capacity || 40,
        academicYear: classRecord.academicYear || '2025',
        isEnglishMedium: classRecord.isEnglishMedium || false,
      });
    } catch (err) {
      console.error(err);
    } finally {
  ...
  // --- Assign unassigned student to this class ---
  const handleAssignStudent = async (student) => {
    if (!cls) return;
    setAssigning(true);
    try {
      await db.students.update(student.id, {
        grade: cls.grade, parallel: cls.parallel, section: cls.section,
      });
      await loadData();
    } finally {
      setAssigning(false);
  ...
  // --- Remove student from class (back to unassigned) ---
  ...
  const openEditStudent = (s) => {
    ...
    setStudentForm({
      ...
      dob: s.dob || '',
      religion: s.religion || 'Buddhism',
      address: s.address || '',
      parentName: s.parentName || '',
      parentPhone: s.parentPhone || '',
      parentEmail: s.parentEmail || '',
      status: s.status || 'Active',
      academicYear: s.academicYear || '2025',
      stream: s.stream || '',
      medium: s.medium || '',
    });
    setStudentError('');
  ...
  const handleSaveStudent = async (e) => {
    ...
    try {
      ...
      await db.students.update(selectedStudent.id, data);
  ...
  const handleSaveClass = async (e) => {
    ...
    try {
      await db.classes.update(cls.id, {
        classTeacherId: classForm.classTeacherId ? Number(classForm.classTeacherId) : null,
        capacity: Number(classForm.capacity),
        academicYear: classForm.academicYear,
        isEnglishMedium: classForm.isEnglishMedium,
      });
  ...
  const handleAssignTeacher = async (e) => {
    ...
    // Prevent duplicate (same teacher + same medium on same subject)
    ...
    try {
      ...
      const newId = await db.subjectTeachers.add(newRecord);
      // Update in-place — no loadData() so the page stays where it is
      setSubjectTeachers(prev => [...prev, { ...newRecord, id: newId }]);
      setAssignTeacherModal(false);
    } catch (err) {
      setTeacherAssignError(err.message);
    } finally {
  ...
  const handleRemoveTeacherAssign = async (stId) => {
    ...
    // Update in-place — no loadData() so the page stays where it is
    setSubjectTeachers(prev => prev.filter(st => st.id !== stId));
  };
  ...
  // Shared student form fields (used in both edit and add modals)
    ...
    <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input
          ...
          className={`w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500 font-mono ${!selectedStudent ? 'bg-slate-50 dark:bg-slate-700/50 text-slate-500' : ''}`}
        />
      </div>
      <div>
      ...
      <div>
        ...
        <select value={studentForm.gender} onChange={e => setStudentForm(f => ({ ...f, gender: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500">
          <option>Male</option><option>Female</option>
        </select>
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Date of Birth</label>
        <input type="date" value={studentForm.dob} onChange={e => setStudentForm(f => ({ ...f, dob: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500" />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Religion</label>
        <select value={studentForm.religion} onChange={e => setStudentForm(f => ({ ...f, religion: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500">
      ...
      <div>
        ...
        <select value={studentForm.status} onChange={e => setStudentForm(f => ({ ...f, status: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500">
      ...
      {cls.section === 'Advanced' && (
        <div>
          ...
          <select value={studentForm.stream} onChange={e => setStudentForm(f => ({ ...f, stream: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500">
      ...
      {cls.isEnglishMedium && (
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">
            Medium <span className="text-xs text-emerald-600 dark:text-emerald-400 font-normal">(English Medium Class)</span>
          </label>
          <select value={studentForm.medium} onChange={e => setStudentForm(f => ({ ...f, medium: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500">
      ...
      <div>
        ...
        <input value={studentForm.academicYear} onChange={e => setStudentForm(f => ({ ...f, academicYear: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500" />
      </div>
      <div className="md:col-span-2">
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Address</label>
        <input value={studentForm.address} onChange={e => setStudentForm(f => ({ ...f, address: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500" />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Parent/Guardian Name</label>
        <input value={studentForm.parentName} onChange={e => setStudentForm(f => ({ ...f, parentName: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500" />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Parent Phone</label>
        <input value={studentForm.parentPhone} onChange={e => setStudentForm(f => ({ ...f, parentPhone: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500" />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Parent Email</label>
        <input type="email" value={studentForm.parentEmail} onChange={e => setStudentForm(f => ({ ...f, parentEmail: e.target.value }))} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500" />
      </div>
    </form>
  );
    ...
    <div className="space-y-5">
      ...
      <div className={`bg-gradient-to-r ${gradient} rounded-2xl p-6 text-white`}>
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            ...
            {teacher ? (
              <div className="flex items-center gap-2 mt-3">
                ...
                <div>
                  <p className="text-xs text-white/60">Class Supervisor</p>
                  <p className="text-sm font-medium">{teacher.name}</p>
                </div>
              </div>
            ) : (
              <p className="text-sm text-white/60 mt-2">No class supervisor assigned</p>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            {/* Edit Class button — admin only */}
            {isAdmin && (
              <button
                onClick={() => { setClassError(''); setEditClassModal(true); }}
                className="flex items-center gap-1.5 px-3 py-2 bg-white/20 hover:bg-white/30 rounded-xl text-sm font-medium transition-colors"
              >
                <Settings size={15} /> Edit Class
              </button>
            )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-2 justify-between">
              <h3 className="font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2 text-sm">
                ...
                <span className="text-xs text-slate-400 dark:text-slate-500 font-normal">({students.length})</span>
              </h3>
              <div className="flex items-center gap-2">
                {/* Import from Excel — admin or class supervisor */}
                {canManage && (
                  <button
                    onClick={openImport}
                ...
                {/* Add from unassigned — admin, principal, or class teacher */}
                {canManage && (
                  <button
            ...
            <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700/50">
              {students.length === 0 ? (
                <div className="px-4 py-8 text-center text-slate-400 dark:text-slate-500 text-sm">No students enrolled yet</div>
              ) : students.map(s => (
                <div key={s.id} className="p-4 flex items-center gap-3">
                  <Avatar name={s.name} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <button onClick={() => { setSelectedStudent(s); setViewModal(true); }} className="font-medium text-slate-800 dark:text-slate-100 text-sm text-left truncate">{s.name}</button>
                      <StatusBadge status={s.status} />
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{s.admissionNo} · {s.gender}</p>
                    <div className="flex items-center gap-1 mt-2">
                      <button onClick={() => { setSelectedStudent(s); setViewModal(true); }} className="p-1 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg" title="View"><Eye size={13} /></button>
                      {isAdmin && <button onClick={() => openEditStudent(s)} className="p-1 text-slate-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg" title="Edit"><Edit size={13} /></button>}
                      {canManage && <button onClick={() => handleRemoveFromClass(s)} className="p-1 text-slate-400 hover:text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg" title="Remove from class"><UserX size={13} /></button>}
                    </div>
                  </div>
                </div>
              ))}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">DOB</th>
                    {cls.isEnglishMedium && <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Medium</th>}
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                  {students.length === 0 ? (
                    <tr><td colSpan={cls.isEnglishMedium ? 8 : 7} className="px-4 py-8 text-center text-slate-400 dark:text-slate-500">No students enrolled yet</td></tr>
                  ) : students.map((s, idx) => (
                    <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <Avatar name={s.name} />
                          <button onClick={() => { setSelectedStudent(s); setViewModal(true); }} className="font-medium text-slate-800 dark:text-slate-100 hover:text-blue-600 dark:hover:text-blue-400 text-left">
                            {s.name}
                          </button>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-slate-500 dark:text-slate-400 text-xs">{s.dob || '—'}</td>
                      {cls.isEnglishMedium && (
                        <td className="px-4 py-3">
                          {s.medium ? (
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${MEDIUM_COLORS[s.medium] || 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}`}>{s.medium}</span>
                          ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          {/* View — everyone */}
                          <button onClick={() => { setSelectedStudent(s); setViewModal(true); }} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="View profile"><Eye size={14} /></button>
                          {/* Edit — admin only */}
                          {isAdmin && (
                            <button onClick={() => openEditStudent(s)} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors" title="Edit student"><Edit size={14} /></button>
                          )}
                          {/* Remove from class — admin or class teacher */}
                          {canManage && (
                            <button onClick={() => handleRemoveFromClass(s)} className="p-1.5 text-slate-400 hover:text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg transition-colors" title="Remove from class"><UserX size={14} /></button>
                          )}
      ...
      {tab === 'subjects' && (
        <div className="space-y-3">
          ...
          {subjects.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-8 text-center text-slate-400 dark:text-slate-500 text-sm">
                ...
                <div key={sub.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500">{sub.type}</p>
                      </div>
                    </div>
                    {canManage && (
                      <button
                        onClick={() => openAssignTeacher(sub)}
                  ...
                  <div className="px-4 py-3 space-y-2">
                    {assignments.length === 0 ? (
                      <p className="text-xs text-slate-400 dark:text-slate-500 italic">No teachers assigned yet</p>
                    ) : assignments.map(st => {
                        ...
                        <div key={st.id} className="flex items-center justify-between gap-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg px-3 py-2">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <span className="text-xs text-slate-400 dark:text-slate-500">{st.medium}</span>
                            </div>
                          </div>
                          {canManage && (
                            <button
                              onClick={() => handleRemoveTeacherAssign(st.id)}
                              className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors shrink-0"
            ...
            // For AL: group by stream; for others: group by subject group
                    ...
                    return (
                      <div key={stream} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                        <div className={`px-4 py-3 border-b border-slate-100 dark:border-slate-800 ${st.header} flex items-center gap-2`}>
                          ...
                          <span className="text-xs text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full ml-1">{streamSubs.length} subjects</span>
                        </div>
                          ...
                          {relSubs.length > 0 && (
                            <div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {relSubs.map(sub => SubjectCard(sub, idx++))}
                              </div>
                            </div>
                          )}
                          {coreSubs.length > 0 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {coreSubs.map(sub => SubjectCard(sub, idx++))}
                            </div>
                          )}
                          {aesSubs.length > 0 && (
                            <div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {aesSubs.map(sub => SubjectCard(sub, idx++))}
                              </div>
                            </div>
                          )}
                        </div>
            ...
            return (
              <div className="space-y-5">
                ...
                {religionSubjects.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">Students take one based on faith</span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {religionSubjects.map((sub, idx) => SubjectCard(sub, idx))}
                    </div>
                  </div>
                )}
                ...
                {/* Core subjects */}
                <div>
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">Core Subjects</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {optionalSubjects.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">Students choose from this category</span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
                        {aestheticsSubjects.map(s => s.name).join(' · ')}
                      </span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {technicalSubjects.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{technicalSubjects.length} subjects</span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/50">
                    <th className="border border-slate-200 dark:border-slate-700 px-3 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 min-w-[80px]">Period</th>
                    {DAYS.map(d => <th key={d} className="border border-slate-200 dark:border-slate-700 px-3 py-2 text-center text-xs font-semibold text-slate-700 dark:text-slate-200 min-w-[100px]">{d}</th>)}
                  </tr>
                </thead>
                <tbody>
      ...
      {/* Edit Class Settings — admin only */}
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setEditClassModal(false)} className="px-4 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">Cancel</button>
            <button onClick={handleSaveClass} disabled={savingClass} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-60 flex items-center gap-2">
              <Save size={14} /> {savingClass ? 'Saving...' : 'Save Changes'}
            </button>
          </>
        }
      >
        {classError && <p className="mb-3 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-3 py-2 rounded-lg">{classError}</p>}
        <div className="space-y-4">
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg px-4 py-3">
            ...
            <p className="font-semibold text-slate-800 dark:text-slate-100">Grade {cls.grade} — {cls.parallel} ({cls.section})</p>
          </div>
          <div>
            ...
            <select
              ...
              onChange={e => setClassForm(f => ({ ...f, classTeacherId: e.target.value }))}
              className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500"
            >
          ...
          {/* English Medium toggle */}
          <button
            type="button"
            onClick={() => setClassForm(f => ({ ...f, isEnglishMedium: !f.isEnglishMedium }))}
            className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border-2 transition-all ${
              classForm.isEnglishMedium
                ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20'
                : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50'
            }`}
          >
            <div className="flex items-center gap-2.5">
              <Globe size={16} className={classForm.isEnglishMedium ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500'} />
              <div className="text-left">
                <p className={`text-sm font-medium ${classForm.isEnglishMedium ? 'text-emerald-700 dark:text-emerald-300' : 'text-slate-700 dark:text-slate-200'}`}>
                  English Medium Class
                </p>
                <p className="text-xs text-slate-400 dark:text-slate-500">Students can select their medium (Sinhala / English / Tamil)</p>
              </div>
            </div>
            {classForm.isEnglishMedium
              ? <ToggleRight size={24} className="text-emerald-500 shrink-0" />
              : <ToggleLeft size={24} className="text-slate-300 dark:text-slate-600 shrink-0" />
            }
          </button>
        </div>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button onClick={handleSaveStudent} disabled={savingStudent} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-60">
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            ...
            <TeacherPicker
              teachers={allTeachers.filter(t => t.status === 'Active')}
              value={teacherAssignForm.teacherId}
              onChange={id => setTeacherAssignForm(f => ({ ...f, teacherId: id }))}
              allowClear={false}
          ...
          <div>
            ...
            <select
              value={teacherAssignForm.medium}
              onChange={e => setTeacherAssignForm(f => ({ ...f, medium: e.target.value }))}
              className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 outline-none focus:border-blue-500"
            >
      ...
      <Modal
        ...
        title="Add Unassigned Students to This Class"
        size="md"
      >
        ...
        {classlessStudents.length === 0 ? (
          <p className="text-slate-400 dark:text-slate-500 text-sm text-center py-6">No unassigned students available.</p>
        ) : (
              ...
              <div key={s.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">{s.admissionNo} · {s.gender}</p>
                  </div>
                </div>
                <button
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">{selectedStudent.admissionNo}</p>
              </div>
            ...
            {isAdmin && (
              <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                <button
                  onClick={() => { setViewModal(false); openEditStudent(selectedStudent); }}
                ...
                <button
                  onClick={() => { setViewModal(false); handleRemoveFromClass(selectedStudent); }}
                  className="flex items-center gap-1.5 px-3 py-2 text-sm border border-orange-300 dark:border-orange-700 text-orange-600 dark:text-orange-400 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/20"
                >
